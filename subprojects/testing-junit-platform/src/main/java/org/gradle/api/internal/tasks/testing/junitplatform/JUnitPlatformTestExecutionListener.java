...
public class JUnitPlatformTestExecutionListener implements TestExecutionListener {
    ...
    private class CurrentRunningTestClass {
        ...
        public void report(TestIdentifier testIdentifier, ReportEntry entry) {
            if (name != null) {
                TestDescriptorInternal testCaseDescriptor = testCases.get(testIdentifier.getUniqueId());
                TestOutputEvent event = new DefaultTestOutputEvent(StdOut, entry.toString() + "\n");
                resultProcessor.output(testCaseDescriptor.getId(), event);
            }
        }
    }
}

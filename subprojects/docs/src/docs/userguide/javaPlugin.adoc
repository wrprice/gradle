...
* Simple pattern (since Gradle 4.7): pattern which has no wildcards at all. In this case, simple class name are supported:
  * `gradle test --tests org.gradle.SomeTestClass`
  * `gradle test --tests org.gradle.SomeTestClass.someSpecificMethod`
  * `gradle test --tests SomeTestClass` (can match `SomeTest` at any package)
  * `gradle test --tests SomeTestClass.someSpecificMethod` (can match `SomeTest.someSpecificMethod` at any package)
* Pattern containing but not starting with a wildcard:
  * `gradle test --tests all.in.specific.package\*`
  * `gradle test --tests all.in.specific.package\*.someSpecificMethod`
* Pattern starting with a wildcard.
...
Therefore, to avoid unnecessary performance loss, please don't use a pattern starting with a wildcard (*).

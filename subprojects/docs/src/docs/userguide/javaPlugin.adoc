...
* Wildcard '*' is supported for matching any characters, however, abusing wildcards might lead to performance problems - see instructions below.
...
Generally speaking, there're three kinds of pattern:

[[simple_pattern]]
===== Simple pattern

A simple pattern has no wildcards at all. In this case, class name or method name should be matched exactly.
...
Since Gradle 4.7, simple class name are supported.

```
# can match 'SomeTest' at any package
gradle test --tests SomeTestClass

# can match 'SomeTest.someSpecificMethod' at any package
gradle test --tests SomeTestClass.someSpecificMethod
```

[[pattern_not_starting_with_wildcard]]
===== Pattern not starting with wildcard

A pattern containing but not starting with a wildcard can help Gradle exclude many classes.
...
[[pattern_starting_with_wildcard]]
===== Pattern starting with wildcard

When a pattern starts with a wildcard, Gradle needs to load all classes to search for a potentially matched method,
which may lead to very bad performance, especially with api:org.gradle.api.tasks.testing.Test#getForkEvery()[`forkEvery`].
Therefore, to avoid unnecessary performance loss, please use a pattern starting with a wildcard (*) as little as possible.

```
gradle test --tests '*SomeTestClass.someSpecificFeature'
